...
- Historical account curve points must prefer order prices on trade dates: buy date uses the buy price as the entry point, sell date uses the sell price to realize P/L, and only no-trade dates use historical closes for mark-to-market. Latest live prices may override cached closes for the current displayed trading day.
